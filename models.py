...
class vae:
    ...
    def reconstruction_loss(self):
            return tf.losses.mean_squared_error(self.target[:,:,0:2],self.last[:,:,0:2])+0.2*tf.losses.sigmoid_cross_entropy(self.target[:,:,2:5],self.last[:,:,2:5])
    ...
    def build(self):
        with tf.variable_scope("vae_model", reuse=tf.AUTO_REUSE):
            ...
            if(self.cell_enc==tfn.LSTMCell):
                latent=tf.concat([state[0].h,state[1].h,state[0].c,state[1].c],axis=-1)
                print("LATENT",latent)
            else:
                latent=tf.concat([state[0],state[1]],axis=-1)
                print("ELSE LATENT ",latent)
            self.z_mean=tf.layers.dense(latent,self.latent_size)
            self.z_std=tf.layers.dense(latent,self.latent_size)
            mu=self.z_mean
            sigma=self.z_std
            ...
            if (self.cell_dec == tfn.LSTMCell):
                latent_state=tfn.LSTMStateTuple(sampled_z,sampled_z)#,tfn.LSTMStateTuple(mu,sigma))
            else:
                latent_state=sampled_z

            res = tf.zeros([self.batch_size, leng, 5])
            print("Second INPUT", res)

            dec_outs, dec_state = tf.nn.dynamic_rnn(
                self.cell_dec(self.latent_size,name="dec"),
                res,
    ...
    def train(self,sess):
        ...
        for e in range(0,EPOCHS):
            ...
            for i in range(0,MAX_ITER):
                ...
                ls,_,summary=sess.run([self.loss,self.optimizer,self.merged],feed_dict={self.input:x_,self.target:y_,self.input_size:tiled_leng})
                print("LOSS:",ls)
